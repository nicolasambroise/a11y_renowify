...
if(typeof nia_headElm !== 'undefined'){
	...
	nia_headBtn = "<button id='checkA11YHeadBtn' title='Liste des en-têtes' aria-expanded='true' type='button'>Liste des en-têtes</button>"
	...
	nia_headElm.forEach(item => {
		...
		if(item_title == "" && item.querySelector('img') && item.querySelector('img').hasAttribute("alt") && item.querySelector('img').getAttribute("alt") != ""){
	...
	nia_headBottomLine.innerHTML = 'Title : [Hn : '+nia_headHn+'] [role="heading" : '+nia_headRole+']<br>'+nia_headBtn+nia_headMsg+'</ul>';
	document.body.appendChild(nia_headBottomLine)
	toggleHeadList()
	document.getElementById("checkA11YHeadBtn").addEventListener("click", toggleHeadList);
	...
	function toggleHeadList(){
		...
		headBtn.setAttribute("aria-expanded", headBtn.getAttribute("aria-expanded") === 'false' ? 'true' : 'false' )
	}
}
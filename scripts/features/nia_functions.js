...
function isItemHasVisibleContent(item) {
  ...
  for (let i = 0; i < item.childNodes.length; i++) {
    if (
      item.childNodes[i].nodeName != '#text' &&
      item.childNodes[i].nodeName != '#comment'
    ) {
      // console.log(item.childNodes[i])
      style_i = window.getComputedStyle(item.childNodes[i]);
      if (
        style_i.display == 'none' ||
        (style_i.width == '1px' &&
          style_i.height == '1px' &&
          style_i.clip == 'rect(1px, 1px, 1px, 1px)' &&
          style_i.display !== 'none' &&
          style_i.visibility !== 'hidden' &&
          style_i.overflow == 'hidden')
      ) {
        ...
        for (let j = 0; j < item.childNodes[i].childNodes.length; j++) {
          if (
            ...
            item.childNodes[i].childNodes[j].nodeName != '#text' &&
            item.childNodes[i].childNodes[j].nodeName != '#comment'
          ) {
            style_j = window.getComputedStyle(item.childNodes[i].childNodes[j]);
            if (
              style_j.display == 'none' ||
              (style_j.width == '1px' &&
                style_j.height == '1px' &&
                style_j.clip == 'rect(1px, 1px, 1px, 1px)' &&
                style_j.display !== 'none' &&
                style_j.visibility !== 'hidden' &&
                style_j.overflow == 'hidden')
            ) {
              // L'enfant n'est pas visible
            } else {
              return true;
...
function isItemHasDirectContent(item) {
  ...
  if (
    ...
    sanitizeText(tempElement.textContent, lang) != ''
  ) {
    return true;
  }
  return false;
...
function setItemToResultList(list, item) {
  if (list == 'crit') {
    ...
    alert('erreur setItemToResultList');
  }
}

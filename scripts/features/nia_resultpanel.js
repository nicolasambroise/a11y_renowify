...
function createResultPanel() {
  ...
  if (result_crit != '') {
    result_crit =
      "<h2 id='result_crit'>Points critiques</h2><ul>" + result_crit + '</ul>';
  }
  if (result_nc != '') {
    result_nc =
      "<h2 id='result_nc'>Points non-conformes</h2><ul>" + result_nc + '</ul>';
  }
  if (result_dev != '') {
    result_dev =
      "<h2 id='result_dev'>Problèmes Techniques</h2><ul>" +
      result_dev +
      '</ul>';
  }
  if (result_nth != '') {
    result_nth =
      "<h2 id='result_nth'>Nice to have</h2><ul>" + result_nth + '</ul>';
  }
  if (result_man != '') {
    result_man =
      "<h2 id='result_man'>A vérifier manuellement</h2><ul>" +
      result_man +
      '</ul>';
  }
  if (
    ...
    result_global =
      result_crit + result_nc + result_dev + result_nth + result_man;
  }
  ...
  let ThirdPart =
    '<p id="result_html5">Validator W3C : <a href="https://validator.w3.org/nu/?doc=' +
    encodeURIComponent(currentUrl) +
    '" target="_blank">lien</a></p>';
  if (!isPreview) {
    ThirdPart =
      '<ul><li id="result_html5">Validator W3C : <a href="https://validator.w3.org/nu/?doc=' +
      encodeURIComponent(currentUrl) +
      '" target="_blank">lien</a></li><li id="result_wave">WAVE : <a href="https://wave.webaim.org/report#/' +
      encodeURIComponent(currentUrl) +
      '" target="_blank">lien</a></li><li id="result_lighthouse">Lighthouse : <a href="https://pagespeed.web.dev/analysis?url=' +
      encodeURIComponent(currentUrl) +
      '" target="_blank">lien</a></li></ul>';
  }

  if (!only_redactor) {
    checkA11YPanel.innerHTML =
      '<div class="panel-header"><h1>Accessibility check</h1></div><div class="panel-body">' +
      result_global +
      '<hr><details class="cmp-accordion"><summary class="cmp-accordion__summary"><h2 class="cmp-accordion__header">Tests automatiques <svg class="icon" viewBox="0 0 24 24" width="24" height="24" aria-hidden="true" focusable="false"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-filter" x="0" y="0"></use></svg></h2></summary><div class="cmp-accordion__panel">' +
      ThirdPart +
      '</div></details></div>';
  } else {
    checkA11YPanel.innerHTML =
  ...
  for (let i = 0; i < result_focus.length; i++) {
    result_focus[i].addEventListener('click', (e) => {
      ...
      if (targetElement && isItemVisible(targetElement)) {
        ...
        alert(
          'Element non visible actuellement, essayez de redimentionner votre fenêtre pour le faire apparaîte ( .' +
            result_focus[i].getAttribute('data-destination') +
            ')'
        );
      }
...
function activateCheckA11YPanel() {
  ...
  function toggleCheckA11YPanel() {
    if (
      document.getElementById('checkA11YPanel').classList.contains('active')
    ) {
      closeCheckA11YPanel();
    } else {
      openCheckA11YPanel();

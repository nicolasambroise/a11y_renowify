...
function check_part_06(){
	...
	if(!only_error){
		...
		if(nia06j2_flag == true) {
		  setItemToResultList("nth","<li><a href='#' data-destination='nia06j2' class='result-focus label-yellow'>06-J</a> : Présence d'une liste simulée avec des tirets</li>");
		}
	}
	...
	if(isAEM && !only_redactor){
		...
		const nia06l2_nodes = document.querySelectorAll('details > summary > *:not(h1):not(h2):not(h3):not(h4):not(h5):not(h6):not([role="heading"]):not(svg[aria-hidden="true"]):not(span.checkA11YSpan):not(span.cmp-accordionItem__icon):not(.filter-subtitle)');
		...
		if(nia06m_nodes && nia06m_nodes.length > 0){
			for(let i = 0; i < nia06m_nodes.length; i++){
				if(isItemVisible(nia06m_nodes[i])){
					...
					else if(nia06m_Adress && nia06m_Adress.length == 1 && nia06m_Adress[0].parentElement.tagName == "LI" && !only_error){nia06m4_flag=true;setItemOutline(nia06m_nodes[i],"yellow","nia06m4","06-M");}
	...
	if(!only_redactor && isAEM){
		...
		if(nia06o2_nodes && nia06o2_nodes.length > 0){
			for(let i = 0; i < nia06o2_nodes.length; i++){
				...
				if(nia06o2_items){
					...
					if(nia06o2_items[0] && nia06o2_items[0].classList.contains("cmp-focus-top") && nia06o2_items.length == 1 && !only_error){
						nia06o7_flag = true;
		...
		if(nia06o3_flag == true) {
			setItemToResultList("dev","<li><a href='#' data-destination='nia06o3' class='result-focus label-yellow'>06-O</a> : Si il n'y a qu'un seul item dans le focus celui-ci ne doit pas être liste 'ul'</li>");	
		}
		if(nia06o4_flag == true) {
			setItemToResultList("dev","<li><a href='#' data-destination='nia06o4' class='result-focus label-orange'>06-O</a> : Si l’intégralité des items ne contiennent qu’un seul élément textuel celui-ci sera dans un 'p' </li>");	
		}
		if(nia06o5_flag == true) {
			setItemToResultList("dev","<li><a href='#' data-destination='nia06o5' class='result-focus label-orange'>06-O</a> : Le premier élément informatif dans le DOM de chaque item doit être le titre</li>");	
		}	
		if(nia06o6_flag == true) {
			setItemToResultList("dev","<li><a href='#' data-destination='nia06o6' class='result-focus label-orange'>06-O</a> : Les items du focus doivent avoir un contenu </li>");	
		}
		if(nia06o7_flag == true) {
			setItemToResultList("man","<li><a href='#' data-destination='nia06o7' class='result-focus label-yellow'>06-O</a> : Vérifier s'il est normal d'avoir un focus-on-top comme seul élément du focus </li>");	
		}
	}
	...
	if(isAEM){
		...
		const nia06q2_nodes = document.querySelectorAll('.cmp-tabs > [role=tablist]:not([aria-label]):not([aria-labelledby])');
		if(!nia06q2_nodes || nia06q2_nodes.length > 0){
		  setItemToResultList("dev","<li><a href='#' data-destination='nia06q2' class='result-focus label-orange'>06-Q</a> : Absence d'attribut aria-label sur le système d'onglet (à ajouter dans le cq_dialog) </li>");
		...
		if(!only_redactor && !only_error){
			const nia06q3_nodes = document.querySelectorAll('.cmp-tabs > [role=tablist] > [role=tab]:not(button), .cmp-tabs > [role=tablist] > [role=tab]:not([aria-controls])');
			if(!nia06q3_nodes || nia06q3_nodes.length > 0){
			  setItemToResultList("nth","<li><a href='#' data-destination='nia06q3' class='result-focus label-yellow'>06-Q</a> : Structurer idéalement les indicateurs d'onglet dans des éléments bouton avec un attribut role=’tab’ ainsi qu’un attribut aria_controls lié avec l’id de son contenu. </li>");
		...
		if(!only_redactor){
			const nia06q4_nodes = document.querySelectorAll('.cmp-tabs > [role=tablist] > [role=tab]:not([aria-selected]), .cmp-tabs > [role=tablist] > [role=tab][aria-selected=false]:not([tabindex="-1"])');
			if(!nia06q4_nodes || nia06q4_nodes.length > 0){
			  setItemToResultList("dev","<li><a href='#' data-destination='nia06q4' class='result-focus label-orange'>06-Q</a> : Chaque item d’onglet actif aura un attribut aria_selected=’true’, sinon il aura la valeur ‘false’ ainsi qu'un attribut tabindex='-1' dans le cas contraire. </li>");
		...
		if(!only_redactor){
			const nia06q5_nodes = document.querySelectorAll('.cmp-tabs > [role=tablist] > [role=tab]:not([id]), .cmp-tabs > [role=tabpanel]:not([aria-labelledby]), .cmp-tabs > [role=tabpanel]:not([tabindex=0])');
			if(!nia06q5_nodes || nia06q5_nodes.length > 0){
			  setItemToResultList("dev","<li><a href='#' data-destination='nia06q5' class='result-focus label-orange'>06-Q</a> : Chaque contenu d’onglet sera dans un element possédant les attributs role=’tabpanel’ , tabindex=’0’ et ainsi qu’un aria_labelledby  faisant référence au titre de l’onglet. </li>");
